    ...
    public class NanoJPEG
    {
        ...
        public void njDecodeSOF()
        {
            ...
            if (nj.ncomp == 3)
            {
                ...
                nj.rgb = new byte[nj.width * nj.height * 4];    //为了支持XNB改为RGBA
            ...
            if (nj.ncomp == 3)
            {
                ...
                for (yy = nj.height; yy != 0; --yy)
                {
                    for (x = 0; x < nj.width; ++x)
                    {
                        ...
                        nj.rgb[prgb++] = 0xFF;  //为了支持XNB，加上Alpha
                    }
                    py += nj.comp[0].stride;
        ...
        {
            return nj.ncomp == 1 ? nj.comp[0].pixels : nj.rgb;  //如果nj.ncomp == 1，XNB会异常
        }
...
namespace liwq
{
    public class JpgDecoder
    {
        ...
        {
            KeyJ.NanoJPEG jpg = new KeyJ.NanoJPEG();
            byte[] buffer = new byte[stream.Length];
            stream.Read(buffer, 0, buffer.Length);
            if (jpg.njDecode(buffer) == KeyJ.nj_result_t.NJ_OK)
            {
                this.Width = jpg.njGetWidth();
                this.Height = jpg.njGetHeight();
                return jpg.njGetImage();
            }
            return null;
        }
    }
